...
def load_style_feature_data(conn) -> pandas.DataFrame:
    ...
    if not _table_exists(conn, STYLE_TABLE):
        raise RuntimeError(f"{STYLE_TABLE} table does not exist")

    full_count = _count_table_rows(conn, STYLE_TABLE)
    if full_count != 1900:
        raise RuntimeError(f"{STYLE_TABLE} expected 1900 rows, found {full_count}")
    ...
    forbidden_family_columns = sorted(
        ...
        if any(token in column.lower() for token in ["h2h_", "poisson", "odds"])
    )
    if forbidden_family_columns:
        errors.append(f"forbidden family columns present: {forbidden_family_columns}")

    style_columns = get_style_feature_columns()
    missing_style = sorted(set(style_columns) - set(df.columns))
    if missing_style:
        errors.append(f"missing style feature columns: {missing_style}")

    if errors:
        raise ValueError("Style experiment data validation failed: " + "; ".join(errors))

    validate_no_final_holdout_loaded(df)
    print("=== Development Style Feature Data ===")
...
def get_style_feature_columns() -> list[str]:
    return STYLE_FEATURE_COLUMNS.copy()
...
def compute_redundancy_diagnostics(
    ...
    for style_feature in style_features:
        for base_feature in base_elo_features:
            pair_df = df[[style_feature, base_feature]].dropna()
            if len(pair_df) < 2:
                continue
            if (
                pair_df[style_feature].std(ddof=0) == 0
                or pair_df[base_feature].std(ddof=0) == 0
            ):
                continue
            corr = pair_df[style_feature].corr(pair_df[base_feature], method="pearson")
            if pandas.isna(corr):
                continue
            rows.append(
                {
    ...
    redundancy_df = redundancy_df.sort_values(
        ["abs_pearson_corr", "style_feature", "base_elo_feature"],
        ascending=[False, True, True],
    ).reset_index(drop=True)

    print("=== Style Redundancy Diagnostics ===")
    ...
    flagged_style_count = int(flagged["style_feature"].nunique())
    print(f"Style features with abs correlation >= 0.95: {flagged_style_count}")
    if flagged_style_count:
        for feature_name in sorted(flagged["style_feature"].unique()):
            max_corr = flagged.loc[
                flagged["style_feature"] == feature_name,
                "abs_pearson_corr",
            ].max()
...
def run_fold(df, fold_config, feature_sets, models) -> list[dict]:
    ...
    model_runs = [
        ...
        ("logistic_elo_style", "logistic", "base_elo_style"),
        ("xgb_elo", "xgb", "base_elo"),
        ("xgb_elo_style", "xgb", "base_elo_style"),
    ]
    rows: list[dict[str, Any]] = []
...
def _print_pair_deltas(
    ...
    style_agg = _aggregate_row(aggregate_df, style_model)
    aggregate_deltas = {
        metric: float(style_agg[metric] - base_agg[metric])
        for metric in [
            "accuracy",
            "log_loss",
            "brier_score",
    ...
    improved_folds = 0
    print(f"{style_model} vs {base_model}")
    print("Aggregate deltas (style minus Elo):")
    ...
    for fold in sorted(results_df["fold"].unique()):
        ...
        style_row = _result_row(results_df, int(fold), style_model)
        delta = float(style_row["log_loss"] - base_row["log_loss"])
...
def print_comparison_and_verdict(results, redundancy_df) -> None:
    ...
    flagged_style_count = 0
    if not redundancy_df.empty:
        flagged_style_count = int(
            redundancy_df.loc[redundancy_df["likely_redundant"], "style_feature"].nunique()
        )
    mostly_redundant = flagged_style_count >= max(1, len(STYLE_FEATURE_COLUMNS) // 2)
    ...
    logistic_acceptance = {
        ...
        "style_not_mostly_redundant": not mostly_redundant,
    }
    ...
    xgb_style_log_loss = float(_aggregate_row(aggregate_df, "xgb_elo_style")["log_loss"])
    ...
    if can_promote:
        verdict = "PROMOTE_STYLE_TO_CANDIDATE"
        explanation = "logistic_elo_style passed all promotion gates."
    elif xgb_improved and xgb_style_log_loss >= logistic_elo_log_loss:
        verdict = "KEEP_STYLE_EXPERIMENTAL"
        explanation = (
            "xgb_elo_style improved its XGB baseline but did not beat logistic_elo."
        )
    elif mostly_redundant:
        verdict = "REJECT_STYLE_EXPERIMENT"
        explanation = "style features are mostly redundant with existing base/Elo features."
    elif logistic_agg["log_loss"] > 0:
        verdict = "REJECT_STYLE_EXPERIMENT"
        explanation = "logistic_elo_style worsened aggregate log_loss."
    elif logistic_deltas["improved_folds"] < len(FOLDS):
        verdict = "REJECT_STYLE_EXPERIMENT"
        explanation = "style log_loss improvement did not appear in both folds."
    elif logistic_agg["brier_score"] > 0.002:
        verdict = "REJECT_STYLE_EXPERIMENT"
        explanation = "style materially worsened aggregate Brier score."
    elif logistic_agg["draw_f1"] < -0.02:
        verdict = "REJECT_STYLE_EXPERIMENT"
        explanation = "style caused a large aggregate draw F1 loss."
    else:
        verdict = "KEEP_STYLE_EXPERIMENTAL"
        explanation = "style did not clear all promotion gates."

    print("=== Phase 7C Verdict ===")
...
def main() -> None:
    ...
    print("=== Tier 3 Phase 7C Style Proxy Experiment ===")
    ...
    df = load_style_feature_data(conn)
    validate_no_final_holdout_loaded(df)
    base_elo_features = get_base_elo_feature_columns(df)
    style_features = get_style_feature_columns()
    _validate_selected_features(df, style_features, "style")
    feature_sets = {
        "base_elo": base_elo_features,
        "base_elo_style": [*base_elo_features, *style_features],
    }
    print(f"Base + Elo feature count: {len(feature_sets['base_elo'])}")
    print(f"Style feature count: {len(style_features)}")
    print(f"Base + Elo + style feature count: {len(feature_sets['base_elo_style'])}")
    ...
    print("No Streamlit, Tier 2 artifact, H2H, Poisson-feature, betting odds, manager, sentiment, injury, deployment, or app work occurred.")
